      ...
      <div className="container mx-auto px-6">
        ...
        >
          <h2 className="text-4xl md:text-5xl mb-6">
            <span className='bg-gradient-to-r from-foreground via-foreground to-primary bg-clip-text text-transparent text-disabled'>Compatibilidad</span>
            {" "}
            <span className="text-primary text-disabled">Técnica</span>
          </h2>
          <p className="text-xl text-muted-foreground max-w-3xl mx-auto text-disabled">
            Hardware verificado y optimizado para LogOS. Plataformas industriales
            probadas que garantizan máximo rendimiento y estabilidad.
          </p>
        </motion.div>
            ...
            >
              <Card className="p-6 bg-card border-border hover:border-primary/50 transition-all duration-300 group text-center h-full">
                ...
                <div className="flex justify-center mb-4">
                  <Badge
                    variant={tech.status === 'Optimizado' ? 'default' : tech.status === 'Verificado' ? 'secondary' : 'outline'}
                    className={tech.status === 'Optimizado' ? 'bg-primary text-primary-foreground' : ''}
                  >
                    {tech.status}
                  </Badge>
                </div>
        ...
        >
          <Card className="p-8 bg-card border-border">
            <h3 className="text-xl text-foreground mb-6 text-center">
            ...
            <div className="grid grid-cols-2 md:grid-cols-2 gap-4">
              {architectures.map((arch, index) => (
                <div
                  key={index}
                  className={`p-4 rounded-lg border text-center transition-all duration-300 ${arch.supported
                    ? 'border-primary/50 bg-primary/5'
                    : arch.coming
                      ? 'border-muted border-dashed bg-muted/20'
                ...
                >
                  ...
                  <div className="text-xs">
                    {arch.supported ? (
                      <span className="text-primary">✓ Soportado</span>
                    ) : arch.coming ? (
        ...
        >
          <p className="text-sm text-muted-foreground">
            ¿Problemas de ejecución? <span className="text-primary">soporte@bylogos.cl</span>
          </p>
        </motion.div>
      </div>
    </section>
